...
class Astronomy extends IPSModuleStrict
{
    private const NREL = 1367.0;
    ...
    public function Create(): void
    {
        ...
        $this->RegisterPropertyBoolean('radiant_power', false);
        $this->RegisterPropertyBoolean('season', false);
        $this->RegisterPropertyBoolean('sunstarsign', false);
        $this->RegisterPropertyBoolean('moonage', false);
        ...
        $this->RegisterPropertyBoolean('goldenhoureveningend', false);
        $this->RegisterPropertyBoolean('bluehourmorningstart', false);
        $this->RegisterPropertyBoolean('bluehourmorningend', false);
        $this->RegisterPropertyBoolean('bluehoureveningstart', false);
        $this->RegisterPropertyBoolean('bluehoureveningend', false);
        $this->RegisterPropertyBoolean('moonculmination', false);
        $this->RegisterPropertyBoolean('moonlowerculmination', false);
        $this->RegisterPropertyBoolean('sunabovehorizon', false);
        $this->RegisterPropertyBoolean('moonabovehorizon', false);
        $this->RegisterPropertyBoolean('twilightphase', false);
        $this->RegisterPropertyBoolean('sunrightascension', false);
        $this->RegisterPropertyBoolean('sundeclination', false);
        $this->RegisterPropertyBoolean('moonrightascension', false);
        $this->RegisterPropertyBoolean('moondeclination', false);
        $this->RegisterPropertyBoolean('sunzodiaclongitude', false);
        $this->RegisterPropertyBoolean('moonzodiaclongitude', false);
        $this->RegisterPropertyBoolean('sunparallacticangle', false);
        $this->RegisterPropertyBoolean('moonparallacticangle', false);
        ...
        $this->RegisterAttributeInteger('nfac', 2); // atmospheric turbidity (2=clear, 4-5=smoggy)
        $this->RegisterAttributeFloat('sunazimut', 0);
        $this->RegisterAttributeFloat('sundirection', 0);
        $this->RegisterAttributeFloat('sunaltitude', 0);
        $this->RegisterAttributeFloat('sundistance', 0);
    ...
    private function ValidateConfiguration()
    {
        ...
        foreach ($timeVariables as $timeVariable) {
            ...
            if ($this->ReadPropertyBoolean($ident) == true) {
                $this->SetupVariable($ident, $name, '~UnixTimestampTime', $position, VARIABLETYPE_INTEGER, true);
            } else {
                $this->SetupVariable($ident, $name, '~UnixTimestampTime', $position, VARIABLETYPE_INTEGER, false);
            }
        }
        ...
        foreach ($floatVariables as $floatVariable) {
            ...
            if ($this->ReadPropertyBoolean($ident) == true) {
                $this->SetupProfile(VARIABLETYPE_FLOAT, $profile, 'Gauge', '', $suffix, 0, 0, 0, 2, []);
                $this->SetupVariable($ident, $name, $profile, $position, VARIABLETYPE_FLOAT, true);
            } else {
                $this->SetupVariable($ident, $name, $profile, $position, VARIABLETYPE_FLOAT, false);
            }
        }
    ...
    public function MoonphasePercent(): float
    {
    ...
    protected function GetMoonPicture(float $mondphase)
    {
        $language = $this->ReadPropertyInteger('language');
    ...
    protected function ResolveMoonImageFile(string $picid): string
    {
        $picturename = $this->ReadPropertyString('picturename');
        $picturemoonselection = $this->ReadPropertyBoolean('picturemoonselection');
        if ($picturemoonselection) {
            ...
            return IPS_GetKernelDir() . $path . DIRECTORY_SEPARATOR . $picturename . $picid . '.' . $filetype;
        }

        $background = $this->ReadPropertyInteger('moonbackground');
        if ($background == 2) {
            return dirname(__DIR__) . DIRECTORY_SEPARATOR . 'imgs' . DIRECTORY_SEPARATOR . 'mondtransparent' . DIRECTORY_SEPARATOR . 'mond' . $picid . '.png';
        }

        return dirname(__DIR__) . DIRECTORY_SEPARATOR . 'imgs' . DIRECTORY_SEPARATOR . 'mond' . DIRECTORY_SEPARATOR . 'mond' . $picid . '.gif';
    }

    protected function ResolveVisualizationMoonImageFile(string $picid): string
    {
        return dirname(__DIR__) . DIRECTORY_SEPARATOR . 'imgs' . DIRECTORY_SEPARATOR . 'mondtransparent' . DIRECTORY_SEPARATOR . 'mond' . $picid . '.png';
    }
    ...
    protected function JDCD(float $JD)
    {
        $day = $this->JDCDay($JD);
    ...
    protected function DHHMS(float $DH)
    {
    ...
    protected function UTLct(float $UH, float $UM, float $US, int $DS, float $ZC, int $GD, int $GM, int $GY)
    {
    ...
    protected function HMSDH(float $Hour, int $Minute, int $Second)
    {
        $A = abs($Second) / 60;
        $B = (abs($Minute) + $A) / 60;
        $C = abs($Hour) + $B;
    ...
    protected function CDJD(float $day, int $month, int $year)
    {
        if ($month < 3) {
            $Y = $year - 1;
            $M = $month + 12;
    ...
    protected function LSTGST(float $LH, int $LM, int $LS, float $L)
    {
    ...
    protected function UTDayAdjust(int $UT, int $G1)
    {
        $UTDayAdjust = $UT;
    ...
    public function Moon_FirstQuarterDate(string $date): array
    {
    ...
    public function Moon_NewmoonDate(string $date): array
    {
        $timestamp = strtotime($date);
        // Datum in Jahre umrechnen
        $year = ((((((date('s') / 60) + date('i')) / 60) + date('G')) / 24) + date('z', $timestamp) - 1) / (365 + (date('L', $timestamp))) + date('Y', $timestamp);
    ...
    public function Moon_FullmoonDate(string $date): array
    {
        $timestamp = strtotime($date);
        // Datum in Jahre umrechnen
        $year = ((((((date('s') / 60) + date('i')) / 60) + date('G')) / 24) + date('z', $timestamp) - 1) / (365 + (date('L', $timestamp))) + date('Y', $timestamp);
    ...
    public function Moon_LastQuarterDate(string $date): array
    {
        $timestamp = strtotime($date);
        // Datum in Jahre umrechnen
        $year = ((((((date('s') / 60) + date('i')) / 60) + date('G')) / 24) + date('z', $timestamp) - 1) / (365 + (date('L', $timestamp))) + date('Y', $timestamp);
    ...
    public function GetVisualizationTile(): string
    {
        ...
        $assets .= 'window.assets.iconSun = "data:image/svg+xml;base64,' . $this->GetVisualizationAssetBase64('icon-sun.svg') . '";' . PHP_EOL;
        $assets .= 'window.assets.iconMoon = "data:image/svg+xml;base64,' . $this->GetVisualizationAssetBase64('icon-moon.svg') . '";' . PHP_EOL;
        $assets .= 'window.assets.iconHorizon = "data:image/svg+xml;base64,' . $this->GetVisualizationAssetBase64('icon-horizon.svg') . '";' . PHP_EOL;
        $assets .= '</script>';
    ...
    protected function GetVisualizationState(): array
    {
        $astronomyInfo = $this->SetAstronomyValues();
        $labels = $this->GetVisualizationLabels();
        $language = $this->ReadPropertyInteger('language') === 1 ? 'de-DE' : 'en-US';
        $moonImagePath = $this->ResolveVisualizationMoonImageFile((string) $astronomyInfo['picid']);
        $moonPhaseText = $this->TranslateMoonPhaseText((string) $astronomyInfo['moonphasetext']);
        $sunDirection = $this->GetDirectionText((int) $astronomyInfo['sundirection']);
        $moonDirection = $this->GetDirectionText((int) $astronomyInfo['moondirection']);
        ...
        return [
            ...
            'panels' => [
                ...
                'position' => [
                    ...
                    'sunRows' => [
                        ['label' => $this->Translate('Altitude'), 'value' => $this->FormatVisualizationFloat((float) $astronomyInfo['sunaltitude'], 2, '°')],
                ...
                'twilight' => [
                    ...
                    'yearImage' => $this->GetVisualizationImageDataUri($astronomyInfo['twilight_year_image_path']),
                    'times' => [
                        'sunrise' => $this->FormatVisualizationTimestamp((int) $astronomyInfo['Sunrise']),
                        'sunset' => $this->FormatVisualizationTimestamp((int) $astronomyInfo['Sunset']),
                        'civilStart' => $this->FormatVisualizationTimestamp((int) $astronomyInfo['CivilTwilightStart']),
                        'civilEnd' => $this->FormatVisualizationTimestamp((int) $astronomyInfo['CivilTwilightEnd']),
            ...
            'metrics' => [
                ...
                'moon' => [
                    ...
                    'altitude' => $this->FormatVisualizationFloat((float) $astronomyInfo['moonaltitude'], 2, '°'),
            ...
            'details' => [
                ...
                $this->Translate('Sun altitude') => $this->FormatVisualizationFloat((float) $astronomyInfo['sunaltitude'], 2, '°'),
                $this->Translate('Sun direction') => $sunDirection,
                $this->Translate('Sun distance') => $this->FormatVisualizationDistance((float) $astronomyInfo['sundistance']),
                $this->Translate('Radiant Power') => $this->FormatVisualizationFloat((float) $this->Radiant_Power(), 0, ' W/m²'),
                $this->Translate('Moon azimuth') => $this->FormatVisualizationFloat((float) $astronomyInfo['moonazimut'], 2, '°'),
                $this->Translate('Moon altitude') => $this->FormatVisualizationFloat((float) $astronomyInfo['moonaltitude'], 2, '°'),
    ...
    protected function GetVisualizationLabels(): array
    {
        return [
            'overview' => $this->Translate('Overview'),
            'moon' => $this->Translate('Moon'),
            'position' => $this->Translate('Position'),
            'twilight' => $this->Translate('Twilight'),
            'details' => $this->Translate('Values'),
            'day' => $this->Translate('Day'),
            'night' => $this->Translate('Night'),
            'currentState' => $this->Translate('Current state'),
            'season' => $this->Translate('Season'),
            'phase' => $this->Translate('Moon phase'),
            'altitude' => $this->Translate('Altitude'),
            'azimuth' => $this->Translate('Azimuth'),
            'direction' => $this->Translate('Direction'),
            'distance' => $this->Translate('Distance'),
            'visibility' => $this->Translate('Visibility'),
            'brightLimbAngle' => $this->Translate('Bright limb angle'),
            'radiantPower' => $this->Translate('Radiant Power'),
            'moonInformation' => $this->Translate('Moon Information'),
            'positionInformation' => $this->Translate('Position Information'),
            'nextPhases' => $this->Translate('Next phases'),
            'sunMoonView' => $this->Translate('Sun and moon position'),
            'twilightDay' => $this->Translate('Twilight day'),
            'twilightYear' => $this->Translate('Twilight year'),
            'twilightTimes' => $this->Translate('Twilight times'),
            'civilStart' => $this->Translate('Civil twilight start'),
            'civilEnd' => $this->Translate('Civil twilight end'),
            'nauticStart' => $this->Translate('Nautical twilight start'),
            'nauticEnd' => $this->Translate('Nautical twilight end'),
            'astronomicStart' => $this->Translate('Astronomical twilight start'),
            'astronomicEnd' => $this->Translate('Astronomical twilight end'),
            'noImage' => $this->Translate('No graphic available'),
            'noView' => $this->Translate('No view available'),
            'sun' => $this->Translate('Sun'),
            'moonTitle' => $this->Translate('Moon'),
            'sunrise' => $this->Translate('Sunrise'),
    ...
    protected function GetVisualizationAssetBase64(string $fileName): string
    {
    ...
    protected function FormatVisualizationTimestamp(int $timestamp): string
    {
        if ($timestamp <= 0) {
            return '--';
        }
    ...
    protected function FormatVisualizationFloat(float $value, int $precision = 2, string $suffix = ''): string
    {
        return number_format($value, $precision, ',', '.') . $suffix;
    }

    protected function FormatVisualizationDistance(float $value): string
    {
        return number_format(round($value), 0, ',', '.') . ' km';
    }

    protected function GetSeasonText(int $season): string
    {
        switch ($season) {
            case 1:
                return $this->Translate('Spring');
            case 2:
                return $this->Translate('Summer');
            case 3:
                return $this->Translate('Autumn');
            case 4:
                return $this->Translate('Winter');
        }

        return '--';
    }

    protected function GetBooleanText(bool $value): string
    {
        return $value ? $this->Translate('Yes') : $this->Translate('No');
    }

    protected function TranslateMoonPhaseText(string $phaseText): string
    {
        $map = [
            'Vollmond' => 'Full Moon',
            'full moon' => 'Full Moon',
            'abnehmender Mond' => 'Decreasing Moon',
            'decreasing moon' => 'Decreasing Moon',
            'Neumond' => 'New Moon',
            'new moon' => 'New Moon',
    ...
    protected function GetDirectionText(int $direction): string
    {
        $directions = [
            0 => 'N',
            1 => 'NNO',
    ...
    protected function CalculateMoonAgeDays(float $moonPhasePercent): float
    {
        $synodicMonth = 29.530588861;
        $ageFactor = fmod($moonPhasePercent + 50, 100) / 100;

        return round($ageFactor * $synodicMonth, 1);
    }
    ...
    protected function GetTimestampParts(int $timestamp): array
    {
        return [
    ...
    protected function CalculateSunSnapshot(int $timestamp): array
    {
        $location = $this->getlocation();
        $parts = $this->GetTimestampParts($timestamp);
        $Latitude = $location['Latitude'];
        $Longitude = $location['Longitude'];
        $ZC = $this->ReadPropertyFloat('UTC');

        $HMSDec = $this->HMSDH(floatval($parts['hour']), $parts['minute'], $parts['second']);
        $GD = floatval($this->LctUT($parts['hour'], $parts['minute'], $parts['second'], $parts['daylightSaving'], $ZC, $parts['day'], $parts['month'], $parts['year'])['GD']);
        $GM = intval($this->LctUT($parts['hour'], $parts['minute'], $parts['second'], $parts['daylightSaving'], $ZC, $parts['day'], $parts['month'], $parts['year'])['GM']);
        $GY = intval($this->LctUT($parts['hour'], $parts['minute'], $parts['second'], $parts['daylightSaving'], $ZC, $parts['day'], $parts['month'], $parts['year'])['GY']);

        $LCH = $this->DHHour($HMSDec);
        $LCM = $this->DHMin($HMSDec);
        $LCS = $this->DHSec($HMSDec);

        $Sunlong = $this->SunLong($LCH, $LCM, $LCS, $parts['daylightSaving'], $ZC, $parts['day'], $parts['month'], $parts['year']);
        $SunlongDeg = $this->DDDeg($Sunlong);
        $SunlongMin = $this->DDMin($Sunlong);
        $SunlongSec = $this->DDSec($Sunlong);
        $SunRAh = $this->DDDH($this->ECRA($SunlongDeg, $SunlongMin, $SunlongSec, 0, 0, 0, $GD, $GM, $GY));
        $SunRAhour = $this->DHHour($SunRAh);
        $SunRAm = $this->DHmin($SunRAh);
        $SunRAs = $this->DHSec($SunRAh);
        $SunDec = $this->ECDec($SunlongDeg, $SunlongMin, $SunlongSec, 0, 0, 0, $GD, $GM, $GY);
        $SunDecd = $this->DDDeg($SunDec);
        $SunDecm = $this->DDmin($SunDec);
        $SunDecs = $this->DDSec($SunDec);
        $SunH = $this->RAHA($SunRAhour, $SunRAm, $SunRAs, $parts['hour'], $parts['minute'], $parts['second'], $parts['daylightSaving'], $ZC, $parts['day'], $parts['month'], $parts['year'], $Longitude);
        $SunHh = $this->DHHour($SunH);
        $SunHm = $this->DHMin($SunH);
        $SunHs = $this->DHSec($SunH);
        $sunazimut = $this->EQAz($SunHh, $SunHm, $SunHs, $SunDecd, $SunDecm, $SunDecs, $Latitude);
        $sunaltitude = $this->EQAlt($SunHh, $SunHm, $SunHs, $SunDecd, $SunDecm, $SunDecs, $Latitude);
        $sundirection = $this->direction($sunazimut);
    ...
    protected function CalculateMoonSnapshot(int $timestamp): array
    {
        $location = $this->getlocation();
        $parts = $this->GetTimestampParts($timestamp);
        $Latitude = $location['Latitude'];
        ...
        $MoonLong = $this->MoonLong($LCH, $LCM, $LCS, $parts['daylightSaving'], $ZC, $parts['day'], $parts['month'], $parts['year']);
        $MoonLat = $this->MoonLat($LCH, $LCM, $LCS, $parts['daylightSaving'], $ZC, $parts['day'], $parts['month'], $parts['year']);
        $Nutation = $this->NutatLong($GD, $GM, $GY);
        $Moonlongcorr = $MoonLong + $Nutation;
        $EcLonDeg = $this->DDDeg($Moonlongcorr);
        $EcLonMin = $this->DDMin($Moonlongcorr);
        $EcLonSec = $this->DDSec($Moonlongcorr);
        $EcLatDeg = $this->DDDeg($MoonLat);
        ...
        $MoonRAh = $this->DHHour($MoonRA);
        $MoonRAm = $this->DHMin($MoonRA);
        $MoonRAs = $this->DHSec($MoonRA);
        $MoonDECd = $this->DDDeg($MoonDec);
        $MoonDECm = $this->DDMin($MoonDec);
        $MoonDECs = $this->DDSec($MoonDec);
        $MoonH = $this->RAHA($MoonRAh, $MoonRAm, $MoonRAs, $parts['hour'], $parts['minute'], $parts['second'], $parts['daylightSaving'], $ZC, $parts['day'], $parts['month'], $parts['year'], $Longitude);
        $MoonHh = $this->DHHour($MoonH);
        $MoonHm = $this->DHMin($MoonH);
        $MoonHs = $this->DHSec($MoonH);
        $moonazimut = $this->EQAz($MoonHh, $MoonHm, $MoonHs, $MoonDECd, $MoonDECm, $MoonDECs, $Latitude);
        $moonaltitude = $this->EQAlt($MoonHh, $MoonHm, $MoonHs, $MoonDECd, $MoonDECm, $MoonDECs, $Latitude);
        $moondirection = $this->direction($moonazimut);
    ...
    protected function FindSunAltitudeCrossing(int $fromTimestamp, int $toTimestamp, float $targetAltitude): ?int
    {
        ...
        $previousValue = $this->CalculateSunSnapshot($previousTimestamp)['altitude'] - $targetAltitude;

        for ($timestamp = $fromTimestamp + $step; $timestamp <= $toTimestamp; $timestamp += $step) {
            $currentValue = $this->CalculateSunSnapshot($timestamp)['altitude'] - $targetAltitude;
            if ($previousValue == 0.0) {
                return $previousTimestamp;
            }
            if (($previousValue < 0 && $currentValue > 0) || ($previousValue > 0 && $currentValue < 0) || $currentValue == 0.0) {
                return $this->RefineSunAltitudeCrossing($previousTimestamp, min($timestamp, $toTimestamp), $targetAltitude);
            }
    ...
    protected function RefineSunAltitudeCrossing(int $fromTimestamp, int $toTimestamp, float $targetAltitude): int
    {
        $low = $fromTimestamp;
        $high = $toTimestamp;
        $lowValue = $this->CalculateSunSnapshot($low)['altitude'] - $targetAltitude;

        while (($high - $low) > 30) {
            $mid = intval(($low + $high) / 2);
            $midValue = $this->CalculateSunSnapshot($mid)['altitude'] - $targetAltitude;
    ...
    protected function FindMoonTransitTimes(int $dayStartTimestamp): array
    {
        $bestMaxTimestamp = $dayStartTimestamp;
        $bestMinTimestamp = $dayStartTimestamp;
        $bestMaxAltitude = -999.0;
        $bestMinAltitude = 999.0;

        for ($timestamp = $dayStartTimestamp; $timestamp < $dayStartTimestamp + 86400; $timestamp += 600) {
            $altitude = $this->CalculateMoonSnapshot($timestamp)['altitude'];
            if ($altitude > $bestMaxAltitude) {
                $bestMaxAltitude = $altitude;
                $bestMaxTimestamp = $timestamp;
            }
            if ($altitude < $bestMinAltitude) {
                $bestMinAltitude = $altitude;
                $bestMinTimestamp = $timestamp;
            }
        }
    ...
    protected function RefineMoonTransit(int $centerTimestamp, bool $searchMaximum): int
    {
        $bestTimestamp = $centerTimestamp;
        $bestAltitude = $this->CalculateMoonSnapshot($centerTimestamp)['altitude'];

        for ($step = 300; $step >= 30; $step = intval($step / 5)) {
            $improved = true;
            while ($improved) {
                $improved = false;
                foreach ([-$step, $step] as $offset) {
                    $candidateTimestamp = $bestTimestamp + $offset;
                    $candidateAltitude = $this->CalculateMoonSnapshot($candidateTimestamp)['altitude'];
                    if (($searchMaximum && $candidateAltitude > $bestAltitude) || (!$searchMaximum && $candidateAltitude < $bestAltitude)) {
    ...
    protected function CalculateParallacticAngle(float $hourAngleHours, float $declinationDegrees, float $latitudeDegrees): float
    {
        $hourAngleRadians = $this->Radians($hourAngleHours * 15);
        $declinationRadians = $this->Radians($declinationDegrees);
        $latitudeRadians = $this->Radians($latitudeDegrees);
        $numerator = sin($hourAngleRadians);
        $denominator = tan($latitudeRadians) * cos($declinationRadians) - sin($declinationRadians) * cos($hourAngleRadians);

        return $this->Degrees(atan2($numerator, $denominator));
    }

    //decimal degrees to seconds

    protected function FormUTCText(float $UTC): array
    {
        $form = [];
        if ($UTC == 14) {
            $form = array_merge_recursive(
    ...
    public function AlexaResponse(): array
    {
        $astronomyinfo = $this->SetAstronomyValues();
    ...
    public function SetAstronomyValues(): array
    {
        $location = $this->getlocation();
        $Latitude = $location['Latitude'];
        $Longitude = $location['Longitude'];
        ...
        if ($this->ReadPropertyBoolean('sunstarsign') == true) {
            $this->SetValueIfIdentExists('sunstarsign', floor($Sunlong / 30) + 1);
        }
        ...
        $this->WriteAttributeFloat('sunazimut', $sunazimut);
        $this->WriteAttributeFloat('sundirection', $SunDazimut);
        $this->WriteAttributeFloat('sunaltitude', $sunaltitude);
        ...
        if ($this->ReadPropertyBoolean('moonstarsign') == true) {
            $this->SetValueIfIdentExists('moonstarsign', floor($MoonLong / 30) + 1);
        }
        ...
        if ($this->ReadPropertyBoolean('daylength') == true) {
            $this->SetValue('daylength', $this->FormatDuration($daylengthseconds));
        }
        if ($this->ReadPropertyBoolean('nightlength') == true) {
        ...
        if ($this->ReadPropertyBoolean('sunabovehorizon') == true) {
            $this->SetValue('sunabovehorizon', $sunabovehorizon);
        }
        if ($this->ReadPropertyBoolean('moonabovehorizon') == true) {
            $this->SetValue('moonabovehorizon', $moonabovehorizon);
        }
        if ($this->ReadPropertyBoolean('twilightphase') == true) {
            $this->SetValue('twilightphase', $twilightphase);
        }
        if ($this->ReadPropertyBoolean('sunrightascension') == true) {
            $this->SetValue('sunrightascension', $SunRAh);
        }
        if ($this->ReadPropertyBoolean('sundeclination') == true) {
            $this->SetValue('sundeclination', $SunDec);
        }
        if ($this->ReadPropertyBoolean('moonrightascension') == true) {
            $this->SetValue('moonrightascension', $MoonRA);
        }
        if ($this->ReadPropertyBoolean('moondeclination') == true) {
            $this->SetValue('moondeclination', $MoonDec);
        }
        if ($this->ReadPropertyBoolean('sunzodiaclongitude') == true) {
            $this->SetValue('sunzodiaclongitude', $Sunlong);
        }
        if ($this->ReadPropertyBoolean('moonzodiaclongitude') == true) {
            $this->SetValue('moonzodiaclongitude', $Moonlongcorr);
        }
        if ($this->ReadPropertyBoolean('sunparallacticangle') == true) {
            $this->SetValue('sunparallacticangle', $sunparallacticangle);
        }
        if ($this->ReadPropertyBoolean('moonparallacticangle') == true) {
    ...
    public function Moon_CurrentFirstQuarter(): array
    {
        // aktuelles Datum in Jahre umrechnen
        $year = ((((((date('s') / 60) + date('i')) / 60) + date('G')) / 24) + date('z') - 1) / (365 + (date('L'))) + date('Y');
        $moonphase = $this->CalculateMoonphase($year);
    ...
    public function Moon_CurrentFullmoon(): array
    {
        // aktuelles Datum in Jahre umrechnen
        $year = ((((((date('s') / 60) + date('i')) / 60) + date('G')) / 24) + date('z') - 1) / (365 + (date('L'))) + date('Y');
        $moonphase = $this->CalculateMoonphase($year);
    ...
    public function Moon_CurrentLastQuarter(): array
    {
        // aktuelles Datum in Jahre umrechnen
        $year = ((((((date('s') / 60) + date('i')) / 60) + date('G')) / 24) + date('z') - 1) / (365 + (date('L'))) + date('Y');
        $moonphase = $this->CalculateMoonphase($year);
    ...
    public function Moon_Newmoon(): array
    {
        // Datum in Jahre umrechnen
        $year = ((((((date('s') / 60) + date('i')) / 60) + date('G')) / 24) + date('z') - 1) / (365 + (date('L'))) + date('Y');
        $moonphase = $this->CalculateMoonphase($year);
    ...
    public function Moon_FirstQuarter(): array
    {
        // Datum in Jahre umrechnen
        $year = ((((((date('s') / 60) + date('i')) / 60) + date('G')) / 24) + date('z') - 1) / (365 + (date('L'))) + date('Y');
        $moonphase = $this->CalculateMoonphase($year);
    ...
    public function Moon_Fullmoon(): array
    {
        // Datum in Jahre umrechnen
        $year = ((((((date('s') / 60) + date('i')) / 60) + date('G')) / 24) + date('z') - 1) / (365 + (date('L'))) + date('Y');
        $moonphase = $this->CalculateMoonphase($year);
    ...
    public function Moon_LastQuarter(): array
    {
        // Datum in Jahre umrechnen
        $year = ((((((date('s') / 60) + date('i')) / 60) + date('G')) / 24) + date('z') - 1) / (365 + (date('L'))) + date('Y');
        $moonphase = $this->CalculateMoonphase($year);
    ...
    public function MoonphaseText(): array
    {
        $mondphase = $this->MoonphasePercent();
        $picture = $this->GetMoonPicture($mondphase);
        $phase = $picture['phase'];
    ...
    protected function Radiant_Power()
    {
    ...
    protected function SunMoonView($sunazimut, $sunaltitude, $moonazimut, $moonaltitude, bool $tileView = false): string
    {
        $canvaswidth = max(320, $this->ReadPropertyInteger('canvaswidth'));
        $canvasheight = max(150, min(190, $this->ReadPropertyInteger('canvasheight')));
        if ($tileView) {
            $canvaswidth = 420;
            $canvasheight = 124;
        }
        ...
        $marginX = 30;
        $baselineY = $canvasheight - 30;
        $innerWidth = max(220, $canvaswidth - ($marginX * 2));
        $skyHeight = max(68, $baselineY - 22);
        $sunX = $marginX + ($innerWidth * ($sunazimut / 360));
        $moonX = $marginX + ($innerWidth * ($moonazimut / 360));
        $sunY = $baselineY - (($skyHeight / 90) * max(-5, min(90, $sunaltitude)));
        $moonY = $baselineY - (($skyHeight / 90) * max(-5, min(90, $moonaltitude)));
        $sunGlowY = max(22, $sunY);
        $moonGlowY = max(22, $moonY);
...
.axis {
  stroke: rgba(195, 220, 245, 0.38);
  stroke-width: 1.5;
}
.grid {
  stroke: rgba(195, 220, 245, 0.14);
  stroke-width: 1;
  stroke-dasharray: 4 5;
            ...
            <body>
            <svg viewBox="0 0 ' . $canvaswidth . ' ' . $canvasheight . '" preserveAspectRatio="xMidYMid meet" aria-label="Sun and moon position">
              <defs>
                <linearGradient id="sky" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stop-color="#10243a"/>
                  <stop offset="100%" stop-color="#19395b"/>
                </linearGradient>
              ...
              <rect x="0" y="0" width="' . $canvaswidth . '" height="' . $baselineY . '" fill="url(#sky)"/>
              <rect x="0" y="' . $baselineY . '" width="' . $canvaswidth . '" height="' . ($canvasheight - $baselineY) . '" fill="url(#ground)"/>
              <path d="M ' . $marginX . ' ' . $baselineY . ' Q ' . ($canvaswidth / 2) . ' 18 ' . ($canvaswidth - $marginX) . ' ' . $baselineY . '" fill="none" class="grid"/>
              <line x1="' . $marginX . '" y1="' . $baselineY . '" x2="' . ($canvaswidth - $marginX) . '" y2="' . $baselineY . '" class="axis"/>
              <line x1="' . ($marginX + ($innerWidth * 0.25)) . '" y1="' . ($baselineY - 8) . '" x2="' . ($marginX + ($innerWidth * 0.25)) . '" y2="' . ($baselineY + 8) . '" class="axis"/>
              <line x1="' . ($marginX + ($innerWidth * 0.5)) . '" y1="' . ($baselineY - 8) . '" x2="' . ($marginX + ($innerWidth * 0.5)) . '" y2="' . ($baselineY + 8) . '" class="axis"/>
              <line x1="' . ($marginX + ($innerWidth * 0.75)) . '" y1="' . ($baselineY - 8) . '" x2="' . ($marginX + ($innerWidth * 0.75)) . '" y2="' . ($baselineY + 8) . '" class="axis"/>
              <text x="' . $marginX . '" y="' . ($baselineY + 24) . '" class="label" text-anchor="middle">N</text>
              <text x="' . ($marginX + ($innerWidth * 0.25)) . '" y="' . ($baselineY + 24) . '" class="label" text-anchor="middle">O</text>
              ...
              <circle cx="' . round($moonX, 1) . '" cy="' . round($moonY, 1) . '" r="9" fill="#eef5ff" stroke="#b8c8de" stroke-width="2"/>
            </svg>
            </body>
            </html>';

        // Direkte Einbettung statt externer Datei unter webfront/user.
        if ($tileView) {
            $framewidth = 100;
            $framewidthtype = '%';
            $frameheight = 132;
        ...
        $HTMLData = '<iframe srcdoc="' . htmlspecialchars($html, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . '" border="0" frameborder="0" scrolling="no" style="width:' . $framewidth . $framewidthtype . '; height:' . $frameheight . $frameheighttype . '; border:0;"></iframe>';
        if (!$tileView && $this->ReadPropertyBoolean('sunmoonview') == true) {
    ...
    protected function LctUT(int $Hour, int $Minute, int $Second, int $DS, float $ZC, int $day, int $month, int $year)
    {
    ...
    protected function UTGST(float $UH, float $UM, float $US, int $GD, int $GM, int $GY)
    {
    ...
    protected function GSTLST(float $GH, int $GM, int $GS, float $L)
    {
    ...
    protected function DMSDD(float $D, int $M, int $S)
    {
        $A = abs($S) / 60;
        $B = (abs($M) + $A) / 60;
        $C = abs($D) + $B;
    ...
    protected function GSTUT(float $GSH, int $GSM, int $GSS, int $GD, int $GM, int $GY)
    {
        $A = $this->CDJD(floatval($GD), intval($GM), intval($GY));
        $B = $A - 2451545;
        $C = $B / 36525;
